...
As soon as you included all libraries you need to create new `Centrifuge` object instance
and call its `.connect()` method to make actual connection to Centrifugo server:
...
Let's also look at configuration parameters available when initializing `Centrifuge` object
instance.
...
messages will be logged into javascript console. Mostly useful for development.
...
it allows to connect to Centrifugo without `token`, `timestamp` etc. And without application
backend! Please, read separate chapter.
...
To subscribe we must use `subscribe` method of `Centrifuge` object instance.
...
* `resubscribe` – called every time resubscribing on subscription succeeds.
* `resubscribe:error` – called every time resubscribing on subscription failed with error.
...
javascript application - so be careful. For this case there is one extra method `.ready()`
that return promise object. This promise resolves after first subscribe attempt finishes
with success or with error. So when you want to call subscribe on channel already subscribed
before you may find this `ready()` method useful:
